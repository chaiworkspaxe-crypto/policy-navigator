...
import psycopg2
from psycopg2.pool import ThreadedConnectionPool # 🌟 [최적화] 연결 풀링
...
# 🌟 [최적화] 글로벌 DB 연결 풀 객체 (최소 1개, 최대 10개 연결 유지)
_db_pool = None
...
def db_session():
    ...
    """
    pool = get_pool()
    conn = pool.getconn() # 🌟 [최적화] 풀에서 연결 가져오기 (매번 새로 연결 안 함)
    try:
        yield conn
        conn.commit()
        ...
        pool.putconn(conn) # 🌟 [최적화] 연결을 끊지 않고 풀에 반납


def init_db():
    """
    Supabase(PostgreSQL)에 필요한 모든 테이블과 인덱스를 생성합니다.
    (기존 채팅 테이블 + 🌟궁극의 정책 데이터 웨어하우스)
    """
    with db_session() as conn:
        with conn.cursor() as cur:
            # 🌟 [Phase 1] Supabase Vector 익스텐션 활성화
            ...
            # 🚀 [핵심] 100% 누락 없는 '궁극의 정책 DB' 테이블 (웨어하우스)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS policies (
                    policy_id TEXT PRIMARY KEY,       -- 고유 식별자 (API ID 등)
                    title TEXT NOT NULL,              -- 정책명
                    provider TEXT NOT NULL,           -- 주관 기관 (예: 국토교통부)
                    category TEXT,                    -- 지원 분야
                    target_audience TEXT,             -- 상세 대상자 설명
                    age_req TEXT,                     -- 연령 조건
                    income_req TEXT,                  -- 소득 조건
                    region_req TEXT,                  -- 지역 조건
                    summary TEXT NOT NULL,            -- 상세 요약본
                    url TEXT,                         -- 신청 링크
                    deadline TEXT,                    -- 마감일
                    is_active BOOLEAN DEFAULT TRUE,   -- 신청 가능 상태
                    embedding VECTOR(1536),           -- 🌟 OpenAI 임베딩 벡터 값
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL
                )
            """)
...
# 🚀 [핵심 4] Phase 4: AI 에이전트 전용 벡터 검색 함수
def search_policies(query_embedding: list, limit: int = 10):
    """
    유저 질문의 벡터값을 받아 우리 DB에서 가장 관련성 높은 정책들을 찾아 반환합니다.
    """
    with db_session() as conn:
        # pgvector 타입 인식을 위해 세션 등록 필수
...
def build_thread_title(content: str, message_type: str) -> str:
    ...
    if message_type == "structured_search":
        ...
        for line in content.splitlines():
            line = line.strip()
            if line.startswith("- 거주지:"): region = line.split(":", 1)[1].strip()
            elif line.startswith("- 추가 정보:"): extra_info = line.split(":", 1)[1].strip()
        if region and extra_info: return shorten(f"{region} / {extra_info}")
        if region: return shorten(region)
    first_line = ""
    for line in content.splitlines():
        stripped = line.strip()
        if stripped: first_line = stripped; break
    if not first_line: return "새 대화"
    return shorten(first_line)

def sanitize_thread_title(title: str) -> str:
    cleaned = " ".join(title.split()).strip()
    if not cleaned: return "새 대화"
    if len(cleaned) > 50: return cleaned[:49].rstrip() + "…"
    return cleaned

def get_default_thread_inputs() -> dict:
    return { "selected_city": "선택하세요", "selected_district": "선택하세요", "selected_dong": "선택 안 함", "birth_year": "", "extra_info": "" }

def ensure_session_row(cur, user_id: str):
    cur.execute("SELECT user_id FROM chat_sessions WHERE user_id = %s", (user_id,))
    if cur.fetchone(): return
    now = now_text()
    cur.execute("INSERT INTO chat_sessions (user_id, active_thread_id, created_at, updated_at) VALUES (%s, %s, %s, %s)", (user_id, None, now, now))
...
def create_thread(user_id: str, title: str = "새 대화", set_active: bool = True) -> str:
    thread_id = str(uuid.uuid4()); now = now_text(); default_inputs = get_default_thread_inputs()
    with db_session() as conn:
        with conn.cursor() as cur:
            ensure_session_row(cur, user_id)
            cur.execute("INSERT INTO chat_threads (thread_id, user_id, title, created_at, updated_at) VALUES (%s, %s, %s, %s, %s)", (thread_id, user_id, sanitize_thread_title(title), now, now))
            cur.execute("INSERT INTO chat_thread_inputs (thread_id, user_id, selected_city, selected_district, selected_dong, birth_year, extra_info, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (thread_id, user_id, default_inputs["selected_city"], default_inputs["selected_district"], default_inputs["selected_dong"], default_inputs["birth_year"], default_inputs["extra_info"], now))
            if set_active: cur.execute("UPDATE chat_sessions SET active_thread_id = %s, updated_at = %s WHERE user_id = %s", (thread_id, now, user_id))
            else: cur.execute("UPDATE chat_sessions SET updated_at = %s WHERE user_id = %s", (now, user_id))
    return thread_id
...
def set_active_thread(user_id: str, thread_id: str):
    ...
    with db_session() as conn:
        with conn.cursor() as cur:
            ensure_session_row(cur, user_id)
            if not thread_exists_for_user(cur, user_id, thread_id): return
...
def rename_thread(user_id: str, thread_id: str, new_title: str):
    now = now_text(); cleaned_title = sanitize_thread_title(new_title)
    with db_session() as conn:
        with conn.cursor() as cur:
            if not thread_exists_for_user(cur, user_id, thread_id): return
...
def list_user_threads(user_id: str) -> list:
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute("""
                SELECT t.thread_id, t.title, t.updated_at, COUNT(m.id) AS message_count, i.selected_city, i.selected_district, i.selected_dong, i.birth_year, i.extra_info
                FROM chat_threads t
                ...
                WHERE t.user_id = %s
                GROUP BY t.thread_id, t.title, t.updated_at, t.created_at, i.selected_city, i.selected_district, i.selected_dong, i.birth_year, i.extra_info
                ORDER BY t.updated_at DESC, t.created_at DESC
            """, (user_id,))
            rows = cur.fetchall()
    return [{"thread_id": r["thread_id"], "title": r["title"], "updated_at": r["updated_at"], "message_count": r["message_count"], "selected_city": r["selected_city"] or "선택하세요", "selected_district": r["selected_district"] or "선택하세요", "selected_dong": r["selected_dong"] or "선택 안 함", "birth_year": r["birth_year"] or "", "extra_info": r["extra_info"] or ""} for r in rows]
...
def save_chat_message(user_id: str, thread_id: str, role: str, content: str, message_type: str = ""):
    now = now_text()
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ensure_session_row(cur, user_id)
            ...
            if not thread_row: return False
            current_title = thread_row["title"]
            cur.execute("INSERT INTO chat_messages (user_id, thread_id, role, content, message_type, created_at) VALUES (%s, %s, %s, %s, %s, %s)", (user_id, thread_id, role, content, message_type, now))
            next_title = current_title
            if role == "user" and current_title in ["", "새 대화", "이전 대화"]: next_title = build_thread_title(content, message_type)
...
def save_thread_inputs(user_id: str, thread_id: str, selected_city: str, selected_district: str, selected_dong: str, birth_year: str, extra_info: str):
    now = now_text()
    with db_session() as conn:
        with conn.cursor() as cur:
            if not thread_exists_for_user(cur, user_id, thread_id): return False
            cur.execute("SELECT thread_id FROM chat_thread_inputs WHERE user_id = %s AND thread_id = %s", (user_id, thread_id))
            if cur.fetchone():
                cur.execute("UPDATE chat_thread_inputs SET selected_city = %s, selected_district = %s, selected_dong = %s, birth_year = %s, extra_info = %s, updated_at = %s WHERE user_id = %s AND thread_id = %s", (selected_city, selected_district, selected_dong, birth_year, extra_info, now, user_id, thread_id))
            else:
                cur.execute("INSERT INTO chat_thread_inputs (thread_id, user_id, selected_city, selected_district, selected_dong, birth_year, extra_info, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (thread_id, user_id, selected_city, selected_district, selected_dong, birth_year, extra_info, now))
    return True

def load_thread_inputs(user_id: str, thread_id: str) -> dict:
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            if not thread_exists_for_user(cur, user_id, thread_id): return get_default_thread_inputs()
            ...
            row = cur.fetchone()
    if not row: return get_default_thread_inputs()
    return { "selected_city": row["selected_city"] or "선택하세요", "selected_district": row["selected_district"] or "선택하세요", "selected_dong": row["selected_dong"] or "선택 안 함", "birth_year": row["birth_year"] or "", "extra_info": row["extra_info"] or "" }
...
def delete_thread(user_id: str, thread_id: str):
    ...
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            if not thread_exists_for_user(cur, user_id, thread_id): return
            cur.execute("SELECT active_thread_id FROM chat_sessions WHERE user_id = %s", (user_id,))
            session_row = cur.fetchone(); was_active_thread = bool(session_row and session_row["active_thread_id"] == thread_id)
            ...
            if was_active_thread:
                cur.execute("SELECT thread_id FROM chat_threads WHERE user_id = %s ORDER BY updated_at DESC, created_at DESC LIMIT 1", (user_id,))
                next_row = cur.fetchone(); next_thread_id = next_row["thread_id"] if next_row else None
...
def consume_daily_request_quota(user_id: str, daily_limit: int) -> dict:
    if not user_id: return { "allowed": False, "daily_limit": daily_limit, "used": 0, "remaining": 0, "usage_date": today_text() }
    target_date = today_text(); now = now_text()
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ensure_session_row(cur, user_id)
            cur.execute("SELECT request_count FROM daily_request_usage WHERE user_id = %s AND usage_date = %s", (user_id, target_date))
            row = cur.fetchone(); current_count = int(row["request_count"] or 0) if row else 0
            ...
            if row: cur.execute("UPDATE daily_request_usage SET request_count = %s, updated_at = %s WHERE user_id = %s AND usage_date = %s", (next_count, now, user_id, target_date))
            else: cur.execute("INSERT INTO daily_request_usage (user_id, usage_date, request_count, created_at, updated_at) VALUES (%s, %s, %s, %s, %s)", (user_id, target_date, next_count, now, now))
    return { "allowed": True, "daily_limit": daily_limit, "used": next_count, "remaining": max(daily_limit - next_count, 0), "usage_date": target_date }

def refund_daily_request_quota(user_id: str, usage_date: str = "") -> bool:
    if not user_id: return False
    target_date = usage_date or today_text(); now = now_text()
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute("SELECT request_count FROM daily_request_usage WHERE user_id = %s AND usage_date = %s", (user_id, target_date))
            row = cur.fetchone(); current_count = int(row["request_count"] or 0) if row else 0
            if current_count <= 0: return False
            cur.execute("UPDATE daily_request_usage SET request_count = %s, updated_at = %s WHERE user_id = %s AND usage_date = %s", (current_count - 1, now, user_id, target_date))
    return True
...
def get_admin_dashboard_stats() -> dict:
    ...
    with db_session() as conn:
        with conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            total_threads = cur.fetchone()["total_threads"]
            cur.execute("SELECT COUNT(*) as blocked_today FROM daily_request_usage WHERE usage_date = %s AND request_count >= 4", (today,))
            blocked_today = cur.fetchone()["blocked_today"]
    return { "total_users": total_users, "total_threads": total_threads, "blocked_today": blocked_today, "today_date": today }
