...
def sync_to_supabase(policies):
    ...
    formatted_data = []
    for p in policies:
        # 🌟 보조금24 API 규격을 우리 DB 규격에 맞춤
        formatted_data.append({
            "id": p.get("SVC_ID", ""),                       
            "title": p.get("SVC_NM", "이름 없음"),      
            "provider": p.get("JRSCT_INSTT_NM", "정부/지자체"),  # agency -> provider
            "category": "일반복지",          
            "target_audience": p.get("TRGTER_INDVDL_NMArray", ""), 
            "age_req": "",                
            "income_req": "",                               
            "region_req": "",              
            "summary": p.get("SVC_PVSN_CN", "내용 없음"),       # description -> summary
...
def fetch_welfare_data():
    ...
    if not PUBLIC_API_KEY:
        print("⚠️ PUBLIC_DATA_API_KEY가 없어 수집을 건너뜁니다. (.env 확인)")
        return
    ...
    while True:
        ...
        try:
            response = requests.get(WELFARE_API_URL, params=params, timeout=15)
